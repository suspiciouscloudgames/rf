...
export function DepthPortalFallback({ config }: DepthPortalFallbackProps) {
  ...
  useFrame(({ camera, gl }) => {
    ...
    const frame = resolvePortalProgress(stage, transition, progress, observationElapsed)
    const darkness = resolvePortalDarkness(stage, transition, progress, observationElapsed)
